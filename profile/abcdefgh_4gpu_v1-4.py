...
def run_benchmark(
    ...
    dtype=jnp.float16,
    warmup_rounds=5,
    output_json="abcdefgh_4gpu_v1-4_stats.json",
    trace_dir="abcdefgh_4gpu_v1-4_trace",
):
    devices = require_4_gpus()

    # V100 FP16 WMMA/tensor-core GEMMs assume contract dims are multiples of 8.
    if jnp.dtype(dtype) == jnp.dtype(jnp.float16) and n % 8 != 0:
        raise ValueError(
            f"n={n} is not divisible by 8; use multiples of 8 for FP16/tensor-core GEMMs "
            "on V100-class hardware."
        )
    ...
    def expr_abcdefgh(a, b, c, d, e, f, g, h):
        x = jnp.linalg.multi_dot((a, b, c, d, e, f, g, h))
    ...
    with mesh:
        ...
        with jax.profiler.StepTraceAnnotation(
            "abcdefgh_fp16_tc_profiled_step", step_num=0
        ):
            y = expr_abcdefgh(A, B, C, D, E, F, G, H)
            y.block_until_ready()
        e = time.perf_counter()
    ...
    result = make_base_result(
        ...
        extra={
            ...
            "jax_settings": {
                ...
                "preferred_accumulation_dtype": (
                    "float32" if jnp.dtype(dtype) == jnp.dtype(jnp.float16) else None
                ),
            },
        },
    )
...
def main():
    ...
    parser.add_argument("--json", type=str, default="abcdefgh_4gpu_v1-4_stats.json")
    parser.add_argument("--trace-dir", type=str, default="abcdefgh_4gpu_v1-4_trace")
