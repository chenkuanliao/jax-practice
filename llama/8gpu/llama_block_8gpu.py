...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark one fp16 Llama block on GPUs 0-7.")
    ...
    parser.add_argument("--json", type=str, default="llama_block_8gpu.json")
    parser.add_argument(
        "--strategy",
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    model_shards = 8 if args.strategy == "tensor_parallel" else 4
...
def main() -> None:
    ...
    if len(devices) < 8:
        raise RuntimeError(
            f"Expected at least 8 visible GPUs, got {len(devices)}. "
    ...
    if args.strategy == "tensor_parallel":
        ...
        strategy_description = "8-way tensor parallelism with replicated sequence"
    else:
        # Rows follow the two CPU NUMA / local NVLink islands from nvidia-smi topo.
        # Columns pair GPUs with direct cross-island NVLinks: 0-4, 1-5, 2-6, 3-7.
        mesh = Mesh(np.array(devices).reshape((2, 4)), axis_names=("seq", "model"))
        ...
        strategy_description = f"2D {seq_parallel_axis} and 4-way tensor parallelism"
    ...
    result = {
        ...
        "name": f"llama_block_8gpu_{args.strategy}",
        "framework": "jax",
        ...
        "topology": "two_4_gpu_numa_nvlink_islands_with_direct_cross_island_pairs",
        "topology_notes": (
            "GPUs 0-3 and 4-7 are separate CPU NUMA/NVLink islands. "
            "The 2x4 seq_model_parallel mesh keeps model shards within each island "
            "and places the seq axis across direct cross-island NVLink pairs."
        ),
        "mesh": {
            "shape": mesh_shape,
